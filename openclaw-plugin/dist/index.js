...
export default defineToolPlugin({
    ...
    tools: (tool) => [
        tool({
            ...
            execute: (_params, config) => config.printers.map((printer) => ({
                id: printer.id,
        ...
        tool({
            ...
            execute: async ({ printerId, filePath, start }, config, context) => clientFor(config, printerId).uploadGcode(filePath, start ?? false, context.signal),
        }),
        tool({
            ...
            execute: async ({ printerId, filename }, config, context) => clientFor(config, printerId).startPrint(filename, context.signal),
        }),
        tool({
            name: "printer_pause",
            label: "Pause Print",
...
function clientFor(config, printerId) {
...
class OctoPrintClient extends PrinterClient {
    ...
    async uploadGcode(filePath, start, signal) {
...
async function fileFormData(filePath, fields) {
